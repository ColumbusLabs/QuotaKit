...
ARTIFACT="$(mktemp -d "${TMPDIR:-/tmp}/quotakit-claude-ownership.XXXXXX")"
APP_BUNDLE="$ROOT/${APP_NAME}.app"
CLI="$APP_BUNDLE/Contents/Helpers/${CLI_EXECUTABLE_NAME}"
APP="$APP_BUNDLE/Contents/MacOS/${APP_EXECUTABLE_NAME}"
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 3 && strcmp(argv[1], "--log-canary") == 0) {
        os_log_t logger = os_log_create("com.columbuslabs.quotakit.verifier", "visibility");
...
LOG_CANARY="quotakit-ownership-log-canary-$(/usr/bin/uuidgen)"
...
ATTRIBUTED_EVENTS="$ARTIFACT/quotakit-attributed-prompt-authorization-events.log"
