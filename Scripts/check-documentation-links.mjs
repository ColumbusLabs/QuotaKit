...
const approvedDocumentationRoots = [
  "docs",
...
function localDocPath(rawLink, baseDirectory, sourcePath) {
  ...
  const isInApprovedDocumentationTree = approvedDocumentationRoots.some((root) =>
    absolutePath === root || absolutePath.startsWith(`${root}${path.sep}`)
  );
  assert(
    isInApprovedDocumentationTree || approvedRootDocumentation.has(absolutePath),
