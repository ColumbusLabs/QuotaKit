...
TEMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/quotakit-cli-installer.XXXXXX")
trap 'rm -rf "$TEMP_DIR"' EXIT

# macOS only: AppleScript evaluates the command as text, never with privileges.
python3 - "$ROOT/bin/install-quotakit-cli.sh" "$TEMP_DIR" <<'PY'
import json
import os
import shlex
import subprocess
...
app_line = 'APP="/Applications/QuotaKit.app"'
assert source.count(app_line) == 1
assert source.count("/usr/bin/osascript") == 1
assert 'HELPER="$APP/Contents/Helpers/QuotaKitCLI"' in source

app = temp / "QuotaKit's spaced \"quoted\" $literal `name`\napp.app"
...
assert "QuotaKit CLI installed." in baseline.stdout
...
for missing in (False, True):
    ...
    for label, functions, inherited in cases:
        ...
        assert failed.stderr == (
            f"QuotaKitCLI helper not found at {helper}. Please reinstall QuotaKit.\n"
        ), label
        assert failed.stdout == "", label
...
assert shlex.split(generated) == [
    ...
    "/bin/ln", "-sf", str(helper), "/usr/local/bin/quotakit",
    "/bin/ln", "-sf", str(helper), "/opt/homebrew/bin/quotakit",
]
...
for prefix in ("local", "homebrew"):
    assert os.readlink(root / prefix / "bin/quotakit") == str(helper)
...
for tool in ("/bin/mkdir", "/bin/ln"):
    ...
    assert not (root / "homebrew/bin/quotakit").is_symlink(), "Continued after an install failure"
