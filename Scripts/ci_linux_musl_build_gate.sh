...
classify_path() {
  ...
  case "$path" in
    Package.swift|Package.resolved)
      require_linux_musl_build "$path" "changes Swift package inputs"
      ;;
    Sources/*)
      require_linux_musl_build "$path" "changes a package source input"
      ;;
    .github/workflows/ci.yml|Scripts/ci_linux_musl_build_gate.sh)
      require_linux_musl_build "$path" "changes the Linux musl CI contract"
      ;;
  esac
}
...
if [[ "$linux_musl_build" == true ]]; then
  ...
  summary_reason="no Linux musl build inputs changed"
fi
