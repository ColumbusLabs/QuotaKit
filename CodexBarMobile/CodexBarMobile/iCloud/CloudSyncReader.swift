...
final class CloudSyncReader: @unchecked Sendable {
    ...
    // MARK: - P6 · Incremental per-provider sync with change tokens

    /// Fetch only the per-provider records that changed since the last
    /// successful sync, apply them to SwiftData, then return the **current
    /// full state** assembled from SwiftData for the new zone plus a legacy
    /// query for devices that haven't migrated yet. Meant to replace
    /// `fetchAllDeviceSnapshots` as the primary pull path once P6 has
    /// stabilised.
    ///
    /// On `.changeTokenExpired` the stored token is cleared and the fetch is
    /// retried with `nil` — semantics-equivalent to a full replay, invisible
    /// to callers.
    ///
    /// The local SwiftData store is the source of truth for the new-zone
    /// side: we upsert/delete rows from the server delta, then read them
    /// back via `SwiftDataBridge.readAllDeviceSnapshots`. The legacy path
    /// remains a plain CKQuery (dual zone) per P5.
    @MainActor
    func fetchAllDeviceSnapshotsIncremental(
        context: ModelContext
    ) async -> MultiDeviceSyncResult {
        let zoneName = CloudSyncConstants.providerZoneName

        // 1. Load persisted token (if any) and fetch deltas.
        ...
        do {
            ...
            print("[CodexBar Sync] token unarchive failed, will do full replay: \(error)")
            token = nil
        }

        var delta = await syncManager.fetchPerProviderZoneChanges(since: token)

        // 2. On token expiry, clear + retry once from nil.
        if delta.tokenExpired {
            try? SwiftDataBridge.saveChangeToken(
                forZone: zoneName, tokenData: nil, context: context)
            delta = await syncManager.fetchPerProviderZoneChanges(since: nil)
        }

        // 3. Apply delta to SwiftData (no-op when upserted+deleted are both empty).
        if !delta.upserted.isEmpty || !delta.deletedRecordNames.isEmpty {
            do {
                try SwiftDataBridge.applyPerProviderDelta(
                    envelopes: delta.upserted,
                    deletedRecordNames: delta.deletedRecordNames,
                    context: context)
            } catch {
                print("[CodexBar Sync] delta apply failed: \(error)")
                // Fall back to full fetch for this cycle.
                return await syncManager.fetchAllDeviceSnapshots()
            }
        }

        // 4. Persist the new token (unchanged on no-op fetch is OK).
        if let newToken = delta.newToken {
            do {
                let data = try NSKeyedArchiver.archivedData(
                    withRootObject: newToken, requiringSecureCoding: true)
                try SwiftDataBridge.saveChangeToken(
                    forZone: zoneName, tokenData: data, context: context)
            } catch {
                print("[CodexBar Sync] token persist failed: \(error)")
            }
        }

        // 5. Read new-zone state from SwiftData.
        let perProviderSnapshots: [SyncedUsageSnapshot]
        do {
            perProviderSnapshots = try SwiftDataBridge.readAllDeviceSnapshots(from: context)
        } catch {
            print("[CodexBar Sync] SwiftData readAll failed: \(error)")
            perProviderSnapshots = []
        }

        // 6. Legacy zone still goes through the full-query path — those
        //    records are monolithic, so change-token would save marginal
        //    bytes and needs a separate token per zone. Defer that work.
        //    The existing fetchAllDeviceSnapshots already priorities
        //    per-provider over legacy, but we want that logic to run on OUR
        //    freshly-assembled perProviderSnapshots (from SwiftData) — so
        //    reach into the same static helper.
        let legacyOnly = await self.fetchLegacyOnlyForIncremental()

        let merged = CloudSyncManager.prioritiseByDevice(
            perProvider: perProviderSnapshots, legacy: legacyOnly)
        if merged.isEmpty {
            // Zone missing + SwiftData empty + legacy empty = truly no data.
            if delta.zoneMissing, perProviderSnapshots.isEmpty, legacyOnly.isEmpty {
                return .empty
            }
            return .empty
        }
    ...
    /// Legacy zones only — used by the incremental reader so it doesn't
    /// re-query the new per-provider zone it just pulled via change tokens.
    private func fetchLegacyOnlyForIncremental() async -> [SyncedUsageSnapshot] {
        let result = await syncManager.fetchLegacyDeviceSnapshots()
        switch result {
        case .success(let snaps): return snaps
        case .empty, .error: return []
        }
    }
