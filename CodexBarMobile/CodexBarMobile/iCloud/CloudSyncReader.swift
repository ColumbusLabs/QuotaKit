...
final class CloudSyncReader: @unchecked Sendable {
    ...
    /// Device-level status records from the provider zone.
    func fetchDeviceStatuses() async -> [SyncDeviceStatus] {
        await self.syncManager.fetchDeviceStatuses()
    }
        ...
        linkages: [ProviderAccountLinkage] = []) -> SyncedUsageSnapshot?
    {
        ...
        let powerStatus = snapshots.count == 1 ? snapshots[0].powerStatus : nil
