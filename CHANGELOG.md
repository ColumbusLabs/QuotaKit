...
- Zed: optionally show browser-based token spend and the reported billing cap while keeping editor Keychain authentication separate.
...
- Codex: prevent growing subagent session rescans from duplicating usage rows and inflating cost estimates.
...
- Codex: publish fully verified current-day token and cost totals while older history remains in bounded catch-up, without replacing established historical totals or reintroducing large-cache memory spikes.
