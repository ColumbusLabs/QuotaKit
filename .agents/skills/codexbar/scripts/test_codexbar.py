...
def make_env(root: Path, *, install: bool = True) -> tuple[dict[str, str], Path]:
    binary = root / "QuotaKit.app" / "Contents" / "Helpers" / "QuotaKitCLI"
    binary.parent.mkdir(parents=True)
    binary.write_text(FAKE, encoding="utf-8")
    binary.chmod(0o755)
    env = os.environ.copy()
    env["QUOTAKIT_SKIP_DISCOVERY"] = "1"
    env["QUOTAKIT_TIMEOUT"] = "3"
    env["FAKE_LOG"] = str(root / "argv.log")
    env["FAKE_VERSION_STDOUT"] = "QuotaKit 1.2.3\n"
    ...
    if install:
        env["QUOTAKIT_BIN"] = str(binary)
    else:
        env.pop("QUOTAKIT_BIN", None)
...
class CodexBarSkillTests(unittest.TestCase):
    def test_help_is_small_and_read_only(self) -> None:
        ...
        self.assertIn("QuotaKit read. JSON out. No writes.", result.stdout)
    ...
    def test_doctor_reports_version_and_raw_validation_shape(self) -> None:
        ...
        self.assertEqual(payload["version"], "QuotaKit 1.2.3")
