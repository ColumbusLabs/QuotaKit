defineProvider({
  ...
  async fetchUsage(ctx) {
    ...
    let creditsResponse;
    try {
      creditsResponse = await ctx.http.get(`${base}/credits`, { headers });
    } catch (error) {
      throw ctx.fail.networkFailure(`OpenRouter network error: ${error && error.message || error}`);
    }
    if (creditsResponse.status === 401) {
      throw ctx.fail.authenticationExpired("OpenRouter API key is invalid or expired.");
    }
    if (creditsResponse.status === 403) {
      throw ctx.fail.permissionDenied("OpenRouter rejected access for this API key.");
    }
    if (creditsResponse.status === 429) throw ctx.fail.rateLimited("OpenRouter API returned HTTP 429.");
    if (creditsResponse.status >= 500) {
      throw ctx.fail.providerUnavailable(`OpenRouter API returned HTTP ${creditsResponse.status}.`);
    }
      ...
      try {
        ...
        if (historyResponse.status !== 200 || latestCompletedResponse.status !== 200) {
          ...
          for (const [index, row] of rows.entries()) {
            ...
            if (date > latestCompleted) throw new TypeError(`activity.data[${index}].date must be a completed UTC day`);
