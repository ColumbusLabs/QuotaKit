defineProvider({
  ...
  async fetchUsage(ctx) {
    ...
    const headers = { "X-Title": ctx.settings.get("OPENROUTER_X_TITLE") || "QuotaKit" };
    ...
    if (!managementKeyConfigured && !primaryManagementKey) {
      ...
      try {
        [historyResponse, latestCompletedResponse] = await Promise.all([
      ...
      if (!activityDegradation && historyResponse && latestCompletedResponse) {
        if (historyResponse.status !== 200 || latestCompletedResponse.status !== 200) {
          ...
          activityDegradation = failed.status === 403
            ? "Management API key required"
            : `Request returned HTTP ${failed.status}`;
        } else {
          try {
            ...
            for (const [index, row] of rows.entries()) {
              ...
              // Activity can report reasoning independently of completion tokens. Keep it separate;
              // the combined token total remains prompt plus completion.
    ...
    let cost = null;
    // A configured cap already has a quota meter. For uncapped keys, preserve the period
    // attached to the available counter and keep account balance separate from spend.
    ...
    if (creditsData) {
      ...
      details.push({
        title: "Credits",
        rows: [
          { label: "Balance", value: "Unavailable right now", secondaryValue: creditsDegradation },
        ],
      });
    }
    ...
    if (!creditsData && !keyData && !costUsage) {
      if (creditsStatus === 401 || keyStatus === 401) {
        throw ctx.fail.authenticationExpired("OpenRouter API error: HTTP 401");
      }
      if (creditsStatus === 403 || keyStatus === 403) {
        throw ctx.fail.permissionDenied("OpenRouter API error: HTTP 403");
      }
