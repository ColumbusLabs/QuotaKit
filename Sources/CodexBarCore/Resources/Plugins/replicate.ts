defineProvider({
  ...
  async fetchUsage(ctx) {
    ...
    const signedOut = (body: string): boolean => {
      const signInTitle = /<title>\s*Sign in\s*\|\s*Replicate\s*<\/title>/i.test(body);
      const githubLogin = /<a\b[^>]*\bhref=["']\/login\/github\/(?:\?[^"']*)?["']/i.test(body);
      return signInTitle && githubLogin;
    };
    const expired = (): never => {
    ...
    const validate = (response: CodexBarHTTPTextResponse): void => {
      if (response.status === 401 || (response.status === 403 && signedOut(response.bodyText))) expired();
      if (response.status === 403) throw ctx.fail.permissionDenied("Replicate denied access to this billing data.");
    ...
    if (!account) {
      ...
      if (signedOut(response.bodyText)) return expired();
