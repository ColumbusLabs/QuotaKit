...
defineProvider({
  ...
  async fetchUsage(ctx) {
    ...
    const organization = _optionalChain([
      ctx,
      "access",
      (_) => _.settings,
      "access",
    ...
    const data = _optionalChain([body, "optionalAccess", (_6) => _6.data]);
    const personal = quota(data);
    if (_optionalChain([body, "optionalAccess", (_7) => _7.error]) != null || !personal) return fail();
    if (
    ...
    const number = (value) => ctx.format.number(value, { maximumFractionDigits: 2 });
