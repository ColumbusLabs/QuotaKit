...
public enum MistralUsageFetcher {
    ...
    static func consoleCookieHeader(csrfToken: String, adminCookieHeader: String?) -> String {
        ...
        if let adminCookies = adminCookieHeader {
            let sessionPairs = CookieHeaderNormalizer.pairs(from: adminCookies)
                .filter(Self.isSafeConsoleSessionCookiePair)
                .map { "\($0.name)=\($0.value)" }
    ...
    private static func isSafeConsoleSessionCookiePair(name: String, value: String) -> Bool {
        guard name.hasPrefix("ory_session_") else { return false }
        let forbidden = CharacterSet(charactersIn: "=;,\r\n")
        guard name.rangeOfCharacter(from: forbidden) == nil else { return false }
        guard !value.isEmpty, value.rangeOfCharacter(from: forbidden) == nil else { return false }
        return true
    }
    ...
    static func parseResponse(data: Data, updatedAt: Date) throws -> MistralUsageSnapshot {
        ...
        let dailyBuckets = try daily.values.map { try $0.makeBucket() }
