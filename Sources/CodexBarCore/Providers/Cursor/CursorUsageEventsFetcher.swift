...
struct CursorUsageEventsFetcher: Sendable {
    ...
    private struct ModelAccumulator {
        ...
        mutating func add(_ usage: CursorEventTokenUsage) {
            ...
            if self.costInvalid {
                // Once a model's running total is invalid, none of its requests can be represented
                // by the published cost. Keep coverage honest even if later rows are well formed.
                self.unpricedRequestCount = self.requestCount
            } else if usage.totalCents == nil {
                self.unpricedRequestCount = Self.checkedSum(self.unpricedRequestCount, 1)
            }
        }
    ...
    private static func makeEntry(date: String, models: [String: ModelAccumulator]) -> CostUsageDailyReport.Entry {
        ...
        for (model, accumulator) in models {
            ...
            unpricedRequestCount = ModelAccumulator.checkedSum([
                unpricedRequestCount,
                accumulator.unpricedRequestCount,
            ])
