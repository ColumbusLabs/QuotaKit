...
public struct ZoomMateUsageFetcher: Sendable {
    ...
    {
        let credentialedHosts = self.apiHosts.filter { cookieHeaders.header(forHost: $0) != nil }
        return try await self.withAPIHostFailover(hosts: credentialedHosts) { host in
