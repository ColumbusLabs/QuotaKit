    ...
    static func makeDescriptor() -> ProviderDescriptor {
        ProviderDescriptor(
            ...
            branding: ProviderBranding(
                ...
                color: ProviderColor(red: 1, green: 210 / 255, blue: 30 / 255),
                confettiPalette: [
...
actor HuggingFaceOperationGate {
    ...
    func release(id: UUID) {
        ...
        while !self.order.isEmpty {
            let next = self.order.removeFirst()
            guard let continuation = self.waiters.removeValue(forKey: next) else { continue }
            self.owner = next
    ...
    func cancel(id: UUID) {
        guard let continuation = self.waiters.removeValue(forKey: id) else { return }
...
final class HuggingFaceScriptFetchStrategy: ProviderFetchStrategy {
    ...
    private let gate = HuggingFaceOperationGate()
