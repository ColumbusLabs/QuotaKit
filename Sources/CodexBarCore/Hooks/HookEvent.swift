...
public struct HookEvent: Codable, Sendable, Equatable {
    ...
    public func environmentVariables() -> [String: String] {
        var values: [String: String] = [
            "QUOTAKIT_EVENT": self.event.rawValue,
            "QUOTAKIT_PROVIDER": self.provider,
            "QUOTAKIT_TIMESTAMP": Self.iso8601String(self.timestamp),
            "CODEXBAR_EVENT": self.event.rawValue,
            "CODEXBAR_PROVIDER": self.provider,
            "CODEXBAR_TIMESTAMP": Self.iso8601String(self.timestamp),
        ]
        func set(_ suffix: String, _ value: String) {
            values["QUOTAKIT_\(suffix)"] = value
            values["CODEXBAR_\(suffix)"] = value
        }
        if let account { set("ACCOUNT", account) }
        if let window { set("WINDOW", window) }
        if let usagePercent { set("USAGE_PERCENT", Self.number(usagePercent)) }
        if let used { set("USED", Self.number(used)) }
        if let limit { set("LIMIT", Self.number(limit)) }
        if let resetAt { set("RESET_AT", Self.iso8601String(resetAt)) }
        if let status { set("STATUS", status) }
        return values
    }
