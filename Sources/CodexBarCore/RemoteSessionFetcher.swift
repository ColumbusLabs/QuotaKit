...
public struct RemoteSessionFetcher: Sendable {
    public static let bundledCLIFallback = "/Applications/QuotaKit.app/Contents/Helpers/QuotaKitCLI"
        ...
        environment: [String: String] = ProcessInfo.processInfo.environment) async
    {
        ...
        let command = Self.focusCommand(sessionID: sessionID)
    ...
    /// wrapper, but a GUI-launched QuotaKit inherits a minimal `PATH` (`/usr/bin:/bin`)
    ...
    public static func focusCommand(sessionID: String) -> String {
        let quotedID = Self.shellQuote(sessionID)
        return "quotakit sessions focus \(quotedID) || " +
            "\(Self.shellQuote(Self.bundledCLIFallback)) sessions focus \(quotedID)"
    }
