...
enum OpenCodexUsageAggregator {
        ...
        customPricingOverlay: CostUsageCustomPricing? = nil) -> CostUsageTokenSnapshot
    {
        ...
        for entry in windowed {
            ...
            session.requests = Self.saturatingAdd(session.requests, 1)
        ...
        into day: inout DayAccumulator)
    {
        ...
        if let cost {
            ...
            day.unpriced = Self.saturatingAdd(day.unpriced, 1)
            if day.priced > 0 {
                day.priced -= 1
            }
    ...
    /// List-price estimate for one entry. Caller rates keep their existing precedence, then exact observed-provider
    /// application overrides and models.dev rates are checked. Only the OpenAI route uses bundled/historical rates.
        ...
        customPricingOverlay: CostUsageCustomPricing) -> Double?
    {
        ...
        let providerID = OpenCodexUsagePricing.providerID(for: entry)
        // Preserve application overrides saved against the exact identity in older OpenCodex logs before routing.
    ...
    private static func add(_ lhs: Int?, _ rhs: Int?) -> Int? {
        switch (lhs, rhs) {
        case let (left?, right?): self.saturatingAdd(left, right)
