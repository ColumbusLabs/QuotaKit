...
public struct CostUsageCustomPricing: Sendable, Equatable {
        ...
        cacheWriteInputTokens: Int) -> Double?
    {
        ...
        let written = min(max(0, cacheWriteInputTokens), remainingAfterCache)
        let uncachedInput = remainingAfterCache - written
        return self.costUSD(
    ...
    private static func rate(_ value: Any?) -> Double? {
        ...
        if let parsed = value as? Double {
            ...
            guard CFGetTypeID(parsed) != CFBooleanGetTypeID() else { return nil }
