...
extension CodexBarCLI {
    ...
    static func runServe(_ values: ParsedValues) async {
        ...
        do {
            try await server.run {
                Self.writeStderr("QuotaKit server listening on http://\(bindHost):\(port)\n")
    ...
    static let dashboardTokenEnvironmentVariable = "QUOTAKIT_DASHBOARD_TOKEN"
    static let legacyDashboardTokenEnvironmentVariable = "CODEXBAR_DASHBOARD_TOKEN"
        ...
        environment: [String: String]) -> CLIServeDashboardTokenResolution
    {
        ...
        if let raw = environment[legacyDashboardTokenEnvironmentVariable] {
            let bearer = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            return bearer.isEmpty
                ? .empty(source: Self.legacyDashboardTokenEnvironmentVariable)
                : .token(bearer)
        }
