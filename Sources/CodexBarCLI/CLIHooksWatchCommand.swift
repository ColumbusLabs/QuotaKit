...
extension CodexBarCLI {
    ...
    /// Config disable checks run during the otherwise idle poll sleep so a long-lived
    /// watcher does not keep executing stale hook commands for the full poll period.
    static let hooksWatchConfigurationCheckNanoseconds: UInt64 = 1_000_000_000
    ...
    static func runHooksWatch(_ values: ParsedValues) async {
        ...
        var revisionTracker = HooksWatchConfigurationRevisionTracker()
        ...
        while !stop.isRequested {
            ...
            guard hooks.enabled else {
                if hasStarted {
                    if !output.usesJSONOutput {
                        print("Hooks were disabled; stopping watch.")
                    }
                    break
                }
                Self.exit(
                    code: .failure,
                    message: "Hooks are disabled. Run `quotakit hooks enable` first.",
                    output: output,
                    kind: .config)
            }
            guard !hooks.events.isEmpty else {
                Self.exit(
                    code: .failure,
                    message: "No hook rules configured. See `quotakit hooks list`.",
                    output: output,
                    kind: .config)
            }
            ...
            do {
                let encoded = try configStore.encodedData(for: config)
                detector.resetIfConfigurationChanged(revision: revisionTracker.revision(for: encoded))
            } catch {
                Self.exit(
                    code: .failure,
                    message: error.localizedDescription,
            ...
            if !hasStarted, !output.usesJSONOutput {
            ...
            let completedSleep = await Self.sleepInterruptibly(
                interval: interval,
                stop: stop,
                shouldContinue: {
                    Self.hooksWatchConfigurationIsEnabled(configStore: configStore)
                })
            if !completedSleep, !stop.isRequested {
                if !output.usesJSONOutput {
                    print("Hooks were disabled; stopping watch.")
                }
                break
        ...
        while remainingNanoseconds > 0, !stop.isRequested {
            ...
            if shouldContinue != nil {
                if sleepNanoseconds >= remainingUntilCheck {
                    ...
                    remainingUntilCheck = checkInterval
                } else {
                    remainingUntilCheck -= sleepNanoseconds
                }
            }
        }
    ...
    /// Reads only the local config file. Decode failures are left for the next full
    /// cycle to report rather than being mistaken for an explicit disable request.
    static func hooksWatchConfigurationIsEnabled(configStore: CodexBarConfigStore) -> Bool {
        guard let config = try? configStore.loadOrCreateDefault() else { return true }
        return (config.hooks ?? HooksConfig()).enabled
    }
...
/// Assigns a monotonic in-process revision to each distinct normalized config.
/// `HookTransitionDetector` uses it to discard stale transition baselines after
/// hook rules, providers, account routing, or warning thresholds change.
struct HooksWatchConfigurationRevisionTracker {
    private var previous: Data?
    private var value = 0

    mutating func revision(for encodedConfig: Data) -> Int {
        guard self.previous != encodedConfig else { return self.value }
        self.previous = encodedConfig
        self.value &+= 1
        return self.value
    }
}
