...
struct DashboardOptions: CommanderParsable {
    ...
    @Option(
        ...
        help: "Account identity detail: redacted (default) or full. "
            + "Full includes real account emails; use it only for snapshots consumed on trusted, private surfaces.")
    var identity: String?
...
/// The CLI command encodes it directly while `quotakit serve` wraps it in the
...
extension CodexBarCLI {
    ...
    static func writeDashboardSnapshotAtomically(_ data: Data, toPath path: String) throws {
        ...
        let staged = directory.appendingPathComponent(
            ".\(url.lastPathComponent).quotakit-dashboard-\(UUID().uuidString)", isDirectory: false)
    ...
    /// `.none` is deliberately not accepted: dashboards redact identity by
    /// default and require an explicit opt-in before exposing full emails.
