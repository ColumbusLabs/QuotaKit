...
extension UsageStore {
        ...
        capturedAt: Date) -> [PlanUtilizationSeriesSample]
    {
        ...
        func appendWindow(_ window: RateWindow?, name: PlanUtilizationSeriesName?) {
            guard let name, let window, let windowMinutes = window.windowMinutes, windowMinutes > 0,
        ...
        case .claude:
            ...
            for scopedWeekly in snapshot.extraRateWindows ?? [] where Self.isClaudeScopedWeeklyWindow(scopedWeekly) {
                appendWindow(scopedWeekly.window, name: PlanUtilizationSeriesName(rawValue: scopedWeekly.title))
            }
        case .antigravity:
