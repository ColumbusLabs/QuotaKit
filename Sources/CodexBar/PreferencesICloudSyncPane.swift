...
struct ICloudSyncPane: View {
    ...
    var body: some View {
        Form {
            if self.state.status.needsAppUpdate {
                Section {
                    Label(
                        L("Sync is paused: another Mac uses a newer version of QuotaKit. Update to resume."),
    ...
    private var availabilityMessage: String? {
        ...
        case .missingEntitlement:
            L("Requires a signed release build of QuotaKit.")
    ...
    private var devices: [DeviceSyncPayload] {
        self.state.fleetDevices.values.sorted { lhs, rhs in
            let lhsIsCurrent = lhs.deviceID == self.settings.macFleetSyncDeviceID
            let rhsIsCurrent = rhs.deviceID == self.settings.macFleetSyncDeviceID
    ...
    private var primarySyncBinding: Binding<Bool> {
        Binding(
            get: { self.settings.macFleetSyncEnabled },
            set: { self.settings.macFleetSyncEnabled = $0 })
    }

    private var includeSecretsBinding: Binding<Bool> {
        Binding(
            get: { self.settings.macFleetSyncIncludeSecrets },
            set: { self.settings.macFleetSyncIncludeSecrets = $0 })
    }

    private var snapshotsBinding: Binding<Bool> {
        Binding(
            get: { self.settings.macFleetSyncSnapshotsEnabled },
            set: { self.settings.macFleetSyncSnapshotsEnabled = $0 })
    }

    private var showFleetAccountsBinding: Binding<Bool> {
        Binding(
            get: { self.settings.macFleetSyncShowFleetAccounts },
            set: { self.settings.macFleetSyncShowFleetAccounts = $0 })
    }
