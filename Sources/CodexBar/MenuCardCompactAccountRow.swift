...
struct MenuCardCompactAccountRowView: View {
    struct Model: Equatable {
        ...
        /// A cached snapshot can coexist with a refresh error. Keep showing the
        /// stale headroom for context, but never let it hide the error marker.
        var showsErrorIndicator: Bool {
            self.hasError
        }

        var showsHeadroomIndicator: Bool {
            self.headroomPercent != nil
        }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 2) {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                ...
                Spacer(minLength: 12)
                if self.model.showsErrorIndicator {
                    Image(systemName: "exclamationmark.triangle")
                        .font(.footnote)
                        .foregroundStyle(MenuHighlightStyle.error(self.isHighlighted))
                        .accessibilityLabel(L("Account unavailable"))
                }
                if self.model.showsHeadroomIndicator,
                   let headroom = self.model.headroomPercent,
                   let label = self.model.headroomLabel
                {
                    Capsule()
