    ...
    static func pin(_ menu: NSMenu, to appearance: NSAppearance?) {
        ...
        for item in menu.items {
            if let submenu = item.submenu {
                // Previously opened submenus may retain an override from an older system appearance.
...
final class StatusMenuAppearanceObserver {
    ...
    private func handleAppearanceChange() {
        ...
        // AppKit may update menu tracking after an appearance change, so re-pin next turn.
        DispatchQueue.main.async { [weak self] in
