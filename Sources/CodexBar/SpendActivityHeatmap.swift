...
struct SpendActivityGridGeometry {
    ...
    /// Caps the tooltip to the grid's height so a very short grid compacts the tooltip instead of
    /// letting it overflow beyond the activity cells.
...
private struct SpendActivityDailyGrid: View {
    ...
    private func accessibilityDescription(at index: Int, date: Date) -> String {
        SpendActivityAccessibility.description(
            date: date,
            value: self.accessibilityTokenValue(at: index),
