...
extension UsageStore {
    ...
    func runResetBoundaryRefresh(boundaryRefreshAt: Date) async {
        ...
        await self.refresh()
        if Self.shouldRecordResetBoundaryAttempt(
    ...
    nonisolated static func shouldRecordResetBoundaryAttempt(
        isRefreshing: Bool,
        snapshots: [UsageProvider: UsageSnapshot],
        boundaryRefreshAt: Date)
        -> Bool
    {
        guard self.shouldRecordResetBoundaryAttempt(isRefreshing: isRefreshing) else { return false }
        return !self.hasStaleResetBoundary(snapshots: snapshots, boundaryRefreshAt: boundaryRefreshAt)
    }
    ...
    private nonisolated static func hasStaleResetBoundary(
        snapshots: [UsageProvider: UsageSnapshot],
        boundaryRefreshAt: Date)
        -> Bool
    {
        snapshots.values.contains { snapshot in
            snapshot.allRateWindows().contains { window in
                guard let resetsAt = window.resetsAt else { return false }
                let candidateBoundary = resetsAt.addingTimeInterval(Self.resetBoundaryRefreshGraceSeconds)
                return abs(candidateBoundary.timeIntervalSince(boundaryRefreshAt)) < 1 &&
