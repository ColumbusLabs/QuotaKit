...
extension UsageStore {
        ...
        input: LimitResetDetectorTransitionInput) -> LimitResetDetectorTransition
    {
        ...
        let nextCodexEarlyWeeklyResetPending = isCodexWeekly && !shouldPost
            && (input.suppressesCodexWeeklyResetCelebration
        ...
        input: LimitResetDetectorTransitionInput) -> Bool
    {
        ...
        if input.provider == .codex, input.seriesName == .weekly {
            guard self.limitResetBoundaryAdvanced(
                previous: input.previousState?.resetBoundary,
                current: input.observation.resetBoundary,
            ...
            guard input.previousState?.codexEarlyWeeklyResetPending == true else { return true }
            return input.previousState?.resetBoundary.map { input.observation.observedAt >= $0 } ?? false
        }
        return true
    }
