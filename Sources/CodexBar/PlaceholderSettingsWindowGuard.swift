...
/// QuotaKit's preferences live in ``SettingsWindowController``. The SwiftUI `Settings` scene is an empty
/// placeholder that only carries the app-menu command group, and because it is the app's only scene macOS
/// presents it during launch, showing an empty "QuotaKit Settings" window (#3053).
enum PlaceholderSettingsWindowDecision {
    ...
    /// A window QuotaKit never wants onscreen: SwiftUI's placeholder Settings window.
...
/// Settings window stays QuotaKit's only preferences surface.
@MainActor
