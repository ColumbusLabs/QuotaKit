...
/// Keep the native menu row on screen; only the cost-history document scrolls when it is tall.
@MainActor
final class CostHistoryMenuScrollView: NSScrollView {
    ...
    static func maximumHeight(for visibleFrameHeight: CGFloat) -> CGFloat {
        min(620, max(1, visibleFrameHeight * 0.72))
    }

    private static func maximumHeight(on screen: NSScreen?) -> CGFloat {
        self.maximumHeight(for: screen?.visibleFrame.height ?? 860)
    }
}
