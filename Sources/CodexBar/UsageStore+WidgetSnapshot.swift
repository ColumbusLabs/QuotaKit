...
extension UsageStore {
    ...
    private func claudeOAuthWidgetQuotaOwnerKey() -> String? {
        let environment = ProviderRegistry.makeEnvironment(
            ...
            tokenOverride: nil)
        guard let accountIdentity = Self.activeClaudeAccountIdentity(environment: environment) else { return nil }
        ...
        snapshot: UsageSnapshot) -> [WidgetSnapshot.WidgetUsageRowSnapshot]
    {
        ...
        case .autoAPI:
            [
                ...
                row(id: "secondary", title: "API", window: snapshot.secondary),
            ]
        case .autoOnly:
            [row(id: "primary", title: "Auto", window: snapshot.primary)]
        case .none:
            if snapshot.cursorRequests != nil || snapshot.detailRow(label: "Request quota") != nil {
                ...
                [
                    row(id: "primary", title: metadata?.sessionLabel ?? "Total", window: snapshot.primary),
                    row(id: "secondary", title: metadata?.weeklyLabel ?? "Auto", window: snapshot.secondary),
                    row(id: "tertiary", title: metadata?.opusLabel ?? "API", window: snapshot.tertiary),
                ]
            } else {
                [
                    row(id: "primary", title: "Auto", window: snapshot.primary),
