...
struct SpendDashboardModel: Equatable, Sendable {
    ...
    private static func bucketCalendar(for provider: UsageProvider, displayCalendar: Calendar) -> Calendar {
        ...
        // These providers label daily buckets and snapshot coverage by UTC day. Map each UTC boundary into the
...
extension SpendDashboardModel.CurrencyGroup {
    ...
    var hasPartialCost: Bool {
        ...
        let incompletePricedCoverage = self.totalCost != nil &&
            (self.coverage.unpriced > 0 || self.coverage.unmetered > 0)
        return mixedProviderTotals || incompletePricedCoverage
    }
