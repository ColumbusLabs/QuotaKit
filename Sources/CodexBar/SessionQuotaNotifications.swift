...
extension UsageStore {
    ...
    func clearSessionQuotaTransitionState(provider: UsageProvider) {
        let removedStates = self.sessionQuotaTransitionStates.filter { $0.key.provider == provider }.map(\.value)
        self.sessionQuotaTransitionStates = self.sessionQuotaTransitionStates.filter { $0.key.provider != provider }
        ...
        if provider == .codex,
           let removedState = removedStates.max(by: { $0.observedAt < $1.observedAt }),
    ...
    func requireFreshCodexSessionQuotaBaseline(observedAt: Date? = nil) {
        let removedStates = self.sessionQuotaTransitionStates.filter { $0.key.provider == .codex }.map(\.value)
        self.sessionQuotaTransitionStates = self.sessionQuotaTransitionStates.filter { $0.key.provider != .codex }
        let removedState = removedStates.max(by: { $0.observedAt < $1.observedAt })
        self.updateCodexSessionQuotaBaselineRequirement(observedAt: removedState?.observedAt)
