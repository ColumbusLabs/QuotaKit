...
struct HooksTests {
    ...
    func `environment variables include set fields and omit nil`() {
        ...
        #expect(env["QUOTAKIT_EVENT"] == "quota_low")
        #expect(env["QUOTAKIT_PROVIDER"] == "codex")
        #expect(env["QUOTAKIT_WINDOW"] == "weekly")
        #expect(env["QUOTAKIT_USAGE_PERCENT"] == "0.5")
    ...
    func `runner executes command and passes event environment`() async throws {
        ...
        #expect(result.stdout.contains("QUOTAKIT_EVENT=quota_reached"))
        #expect(result.stdout.contains("QUOTAKIT_PROVIDER=codex"))
