...
QuotaKit reads Perplexity account credit data with a Perplexity web session cookie. It does not use a Perplexity API
...
```bash
quotakit usage --provider perplexity --verbose
```
...
Sign in to [perplexity.ai](https://www.perplexity.ai/) and refresh QuotaKit, or paste a fresh cookie/session token in
manual mode.
