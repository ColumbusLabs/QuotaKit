...
QuotaKit reads Mistral billing usage with the Mistral web session from `admin.mistral.ai`. It can also fetch credit
...
QuotaKit requests the current UTC month from Mistral Admin:
...
When a CSRF token is available, QuotaKit also makes a bounded best-effort request for Mistral Vibe plan usage:

- `GET https://console.mistral.ai/api-ui/trpc/billing.vibeUsage?...`

For the console request, QuotaKit forwards only the `csrftoken` and `ory_session_*` cookies. Other
...
```bash
quotakit usage --provider mistral --verbose
```
...
does not expose data for the account, QuotaKit keeps the main Mistral usage result.

## Related Files

- `Sources/QuotaKitCore/Providers/Mistral/MistralProviderDescriptor.swift`
- `Sources/QuotaKitCore/Providers/Mistral/MistralUsageFetcher.swift`
- `Sources/QuotaKitCore/Providers/Mistral/MistralModels.swift`
- `Sources/QuotaKitCore/Providers/Mistral/MistralCookieImporter.swift`
- `Sources/QuotaKit/Providers/Mistral/MistralProviderImplementation.swift`
