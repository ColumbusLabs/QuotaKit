...
DeepInfra is API-only. QuotaKit uses DeepInfra's documented billing endpoints and does not send model prompts or account data to any other service.

## Setup

1. Create an API key in the [DeepInfra dashboard](https://deepinfra.com/dash).
2. In QuotaKit, open **Settings > Providers > DeepInfra > API tokens** and add the key.

You can instead set `DEEPINFRA_API_KEY` or `DEEPINFRA_TOKEN` in QuotaKit's environment. `DEEPINFRA_API_KEY` takes precedence when both are present.
...
DeepInfra represents prepaid funds as a negative `stripe_balance`; QuotaKit converts that to a positive “available” amount. A positive value is shown as money owed.
...
- If the account has a positive spending limit, QuotaKit shows billing-cycle spend against that limit.
...
QuotaKit never logs the API key or raw billing response.
