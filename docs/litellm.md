...
read_when:
  ...
  - Troubleshooting LiteLLM API-key usage in QuotaKit
---
...
Configure it in Settings -> Providers -> LiteLLM, or in `~/.quotakit/config.json`:
...
`LITELLM_BASE_URL` may include `/v1`; QuotaKit strips that suffix before calling LiteLLM management endpoints.
...
All requests use `Authorization: Bearer <apiKey>`. QuotaKit does not request or store a LiteLLM master key.
...
Budget tracking requires access to `/key/info` and the corresponding user or team information endpoint. QuotaKit
validates returned user and team IDs against `/key/info` before displaying budget usage.

### Spend-only deployments

When `/key/info` returns HTTP 401, 403, or 404, QuotaKit tries `GET /key/spend/report`. If that report also returns
401, 403, or 404, it tries `GET /user/spend/report`. These routes are deployment-dependent; the virtual key must be
permitted to read a self-scoped report. QuotaKit sends only `start_date` (the first day of the current UTC month) and
...
Treat LiteLLM keys as secrets. QuotaKit stores configured keys only in provider config or token-account storage and
