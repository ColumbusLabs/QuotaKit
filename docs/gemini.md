...
- Token refresh:
  ...
  - Values are form-encoded so literal plus signs and other reserved characters remain intact.
...
- [Google stopped serving](https://developers.google.com/gemini-code-assist/docs/deprecations/code-assist-individuals)
  ...
  subscriptions remain supported; paid API-key access is outside QuotaKit's OAuth-backed Gemini provider.
- When quota, `loadCodeAssist`, or token-refresh responses include Google's unsupported-client
  ...
  QuotaKit surfaces `consumerTierDeprecated` with guidance to use the Antigravity provider.
- Google's live shape is an HTTP **200** `loadCodeAssist` body with no `currentTier` and the consumer tier
  ...
  call then fails with HTTP 403 `SUBSCRIPTION_REQUIRED` and no migration wording. QuotaKit reads the
...
- Settings shows an **Enable Antigravity provider** action only after QuotaKit observes
  `consumerTierDeprecated` during a Gemini refresh (typed sentinel state, not user-facing text matching).
- The action is explicit: QuotaKit never automatically enables Antigravity or falls back to it.
- Ordinary Gemini login, `notLoggedIn`, and Antigravity setup errors remain unchanged. QuotaKit does not
