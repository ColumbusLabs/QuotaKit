...
1. **Authentication** — Three methods (in priority order):
   - **Auto mode**: Username + password entered in Settings → Providers → StepFun.
     QuotaKit performs a 3-step login flow to obtain an Oasis-Token:
...
- `Sources/QuotaKitCore/Providers/StepFun/StepFunProviderDescriptor.swift` (descriptor + web fetch strategy)
- `Sources/QuotaKitCore/Providers/StepFun/StepFunUsageFetcher.swift` (login flow + HTTP client + JSON parser)
- `Sources/QuotaKitCore/Providers/StepFun/StepFunSettingsReader.swift` (env var resolution)
- `Sources/QuotaKit/Providers/StepFun/StepFunProviderImplementation.swift` (settings fields + activation logic)
- `Sources/QuotaKit/Providers/StepFun/StepFunSettingsStore.swift` (SettingsStore extension)
- `Tests/QuotaKitTests/StepFunUsageFetcherTests.swift` (26 test cases)
