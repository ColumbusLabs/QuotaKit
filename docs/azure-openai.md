...
QuotaKit's Azure OpenAI provider validates that a configured deployment is reachable. It does not read Azure spend,
...
QuotaKit sends a minimal chat-completions request to validate the deployment:
...
Set `AZURE_OPENAI_API_VERSION` to override the API version. When it is set to `v1`, QuotaKit uses Azure's
...
HTTPS. QuotaKit rejects explicit `http://` endpoints, user info, and encoded host-delimiter tricks before attaching the
`api-key` header.

Endpoint paths are preserved. QuotaKit avoids duplicating a trailing `/openai` for dated API versions or a trailing
