...
```bash
printf '%s' "$NEURALWATT_API_KEY" | quotakit config set-api-key --provider neuralwatt --stdin
```

This trims the piped key, writes it to QuotaKit's config file (`~/.config/quotakit/config.json`
by default, or the legacy `~/.quotakit/config.json` when already present), and enables Neuralwatt by
...
5. Paste the key into QuotaKit's Neuralwatt provider settings

### Environment Variables

QuotaKit also accepts these environment variables:
...
Set the key with `quotakit config set-api-key --provider neuralwatt --stdin`, add it in
...
quota endpoint is rate-limited to 1 request per second per customer; QuotaKit refreshes on its
