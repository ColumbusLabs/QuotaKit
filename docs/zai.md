...
- XDG override: `$XDG_CONFIG_HOME/quotakit/config.json`
- Override for scripts/tests: `QUOTAKIT_CONFIG=/path/to/config.json`
- Migrated installs: `CODEXBAR_CONFIG` and `~/.codexbar/config.json` remain compatibility fallbacks.
...
- Check:
  ...
  quotakit usage --provider zai --account Team
  ```
...
For BigModel China team usage, QuotaKit needs the `Bigmodel-Organization` and `Bigmodel-Project` request headers:
...
- Endpoint overrides must be explicit HTTPS URLs or bare hosts/paths that QuotaKit normalizes to HTTPS. Explicit
...
- Live API checks return success with empty limits when one of the selectors is missing, so QuotaKit treats both
...
- QuotaKit's Usage Dashboard action follows the configured API region.
