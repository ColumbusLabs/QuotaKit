...
QuotaKit imports your browser session cookie automatically. QuotaKit sends the cookie only to
`https://t3.chat`.
...
5. Paste the full `curl` command into the **T3 Chat cookie** field in QuotaKit settings.
...
```bash
# Show T3 Chat usage
quotakit usage --provider t3chat

# Or use the alias
quotakit usage --provider t3-chat
quotakit usage --provider t3
```
...
| `T3 Chat session cookie is invalid or expired` | Stale or revoked session cookie | Sign out of T3 Chat, sign back in, then refresh QuotaKit or repaste the cookie |
...
| `Could not parse T3 Chat usage` | T3 Chat changed its tRPC response shape | Open a QuotaKit issue with a redacted response sample |
