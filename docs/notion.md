...
> **Unsupported integration:** QuotaKit uses Notion's internal, cookie-authenticated `/api/v3` endpoints.
...
workspaces make the endpoint answer `{"status":"not_applicable"}`, and QuotaKit surfaces that as a clear
...
QuotaKit imports your browser session cookie automatically and sends it only to `https://app.notion.com`.
...
5. Paste the full `curl` command into the **Notion cookie** field in QuotaKit settings.
...
QuotaKit sends two POST requests per refresh, both to `https://app.notion.com`:
...
| QuotaKit window | Notion field | Notes |
| --- | --- | --- |
...
Both bars carry an expected-usage estimate on the card and in `quotakit usage`, reading `n% in deficit`
...
Notion publishes a status page at <https://status.notion.so/>; QuotaKit links to it but does not poll
components.
