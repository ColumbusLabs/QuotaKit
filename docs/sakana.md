...
[Sakana AI](https://sakana.ai) is a research lab focusing on foundation models and nature-inspired AI. QuotaKit reads
...
4. In QuotaKit, paste the header in **Settings → Providers → Sakana AI → Cookie header**.
   The value is stored unencrypted in the [resolved config file](configuration.md#location). QuotaKit sets that file's
...
```
quotakit usage --provider sakana
quotakit usage --provider sakana-ai   # alias
```

Set the cookie via the environment variable or Settings UI:

- **Environment variable**: `SAKANA_COOKIE=<cookie-header-value> quotakit usage --provider sakana`
- **Settings UI**: Settings → Providers → Sakana AI → Cookie header

There is no `quotakit config set` command for `cookieHeader`; use one of the paths above.
