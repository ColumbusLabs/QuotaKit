---
summary: "QuotaKit config file layout for CLI + app settings."
read_when:
  - "Editing the QuotaKit config file or moving settings off Keychain."
...
event's `QUOTAKIT_*` variables and legacy `CODEXBAR_*` aliases; QuotaKit provider keys and tokens are not inherited. The same event is also encoded as
...
The v1 field names and meanings are compatibility-stable. Hook consumers should ignore unknown fields so QuotaKit can
...
quotakit usage --provider example-provider --verbose
```
...
Manual cookies are secrets. Keep the QuotaKit config file private, leave its permissions at `0600`, never commit it,
