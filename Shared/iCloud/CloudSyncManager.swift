...
public final class CloudSyncManager: SyncPushing, @unchecked Sendable {
    ...
    private func ensureCustomZoneExists() async throws {
        ...
        do {
            _ = try await self._privateDatabase!.recordZone(for: self.customZone.zoneID)
            return
        ...
        _ = try await self._privateDatabase!.modifyRecordZones(
            saving: [self.customZone], deleting: [])
    ...
    private func pushToCloudKit(snapshot: SyncedUsageSnapshot, data: Data) async -> SyncPushResult {
        ...
        do {
            try await self.ensureCustomZoneExists()
        } catch {
        ...
        let record: CKRecord
        do {
            record = try await self._privateDatabase!.record(for: recordID)
        ...
        do {
            ...
            do {
                try await self._privateDatabase!.save(serverRecord)
        ...
        _ envelopes: [ProviderUsageEnvelope]) async -> SyncPushResult
    {
        ...
        for chunkStart in stride(from: 0, to: records.count, by: batchSize) {
            ...
            if let failure = await self.saveProviderZoneChunk(
                chunk, operationName: "Per-provider")
            {
                return failure
            }
        }
    ...
    /// Pushes device-level status to `DeviceProvidersZone` under a stable
    /// `deviceID` record name. This is deliberately separate from provider
    /// envelopes so status-only changes are not swallowed by provider-content
    /// diffing in `SyncCoordinator`.
    @discardableResult
    public func pushDeviceStatus(_ status: SyncDeviceStatus) async -> SyncPushResult {
        guard self.cloudKitAvailable, self._privateDatabase != nil else {
            return .failure("CloudKit not available")
        }
        ...
        do {
            let record = try self.makeDeviceStatusRecord(from: status)
            if let failure = await self.saveProviderZoneChunk(
                [record], operationName: "Device-status")
            {
                return failure
            }
            self.logInfo("Pushed device status to CloudKit", metadata: [
                "deviceID": status.deviceID,
                "hasPowerStatus": "\(status.powerStatus != nil)",
            ])
            return .success
        } catch {
            self.logError("Device-status encode failed: \(error.localizedDescription)")
            return .failure(error.localizedDescription)
        }
    }
    ...
    {
        guard self.cloudKitAvailable, self._privateDatabase != nil else { return [] }
        ...
        do {
            let (results, _) = try await _privateDatabase!.records(
                matching: query,
                inZoneWith: self.providerZone.zoneID,
    ...
    /// Encodes a device-level status record. Record name is exactly the stable
    /// Mac `deviceID`, so each Mac overwrites its own current status.
    private func makeDeviceStatusRecord(from status: SyncDeviceStatus) throws -> CKRecord {
        let json = try encoder.encode(status)
        let compressed = try PayloadCompression.compress(json)

        let recordID = CKRecord.ID(recordName: status.deviceID, zoneID: self.providerZone.zoneID)
        let record = CKRecord(
            recordType: CloudSyncConstants.deviceStatusRecordType,
            recordID: recordID)
        record["deviceID"] = status.deviceID as CKRecordValue
        record["deviceName"] = status.deviceName as CKRecordValue
        record["appVersion"] = (status.appVersion ?? "") as CKRecordValue
        record["syncTimestamp"] = status.syncTimestamp as CKRecordValue
        record["encodingVersion"] = CloudSyncConstants.deviceStatusPayloadVersion as CKRecordValue
        record["payload"] = compressed as CKRecordValue
        return record
    }
    ...
    private func saveProviderZoneChunk(
        _ records: [CKRecord],
        operationName: String) async -> SyncPushResult?
    {
        do {
            ...
            let syncError = CloudSyncError(from: error)
            self.logError("\(operationName) batch save failed: \(syncError.description)")
            return .failure(syncError.description)
        } catch {
            self.logError("\(operationName) batch save failed: \(error.localizedDescription)")
            return .failure(error.localizedDescription)
        }
    }
    ...
    public struct PerProviderZoneChanges: Sendable {
        ...
        /// Device-level status records decoded from the same zone.
        public let upsertedDeviceStatuses: [SyncDeviceStatus]
        /// Composite recordNames of records the server reports as deleted.
        public let deletedRecordNames: [String]
        /// Device IDs whose status records were deleted.
        public let deletedDeviceStatusIDs: [String]
        ...
        public init(
            ...
            tokenExpired: Bool,
            zoneMissing: Bool,
            upsertedDeviceStatuses: [SyncDeviceStatus] = [],
            deletedDeviceStatusIDs: [String] = [])
        {
            self.upserted = upserted
            self.upsertedDeviceStatuses = upsertedDeviceStatuses
            self.deletedRecordNames = deletedRecordNames
            self.deletedDeviceStatusIDs = deletedDeviceStatusIDs
        ...
        since token: CKServerChangeToken?) async -> PerProviderZoneChanges
    {
        ...
        nonisolated(unsafe) var deviceStatuses: [SyncDeviceStatus] = []
        nonisolated(unsafe) var deleted: [String] = []
        nonisolated(unsafe) var deletedDeviceStatuses: [String] = []
        nonisolated(unsafe) var capturedToken: CKServerChangeToken? = token

        op.recordWasChangedBlock = { _, result in
            switch result {
            case let .success(record):
                if record.recordType == CloudSyncConstants.providerRecordType,
                   let envelope = Self.decodeEnvelopeStatic(from: record)
                {
                    upserted.append(envelope)
                } else if record.recordType == CloudSyncConstants.deviceStatusRecordType,
                          let status = Self.decodeDeviceStatusStatic(from: record)
                {
        ...
        op.recordWithIDWasDeletedBlock = { recordID, recordType in
            if recordType == CloudSyncConstants.providerRecordType {
                deleted.append(recordID.recordName)
            } else if recordType == CloudSyncConstants.deviceStatusRecordType {
                deletedDeviceStatuses.append(recordID.recordName)
            }
        }
        ...
        self.logInfo("Per-provider zone changes fetched", metadata: [
            ...
            "deviceStatuses": "\(deviceStatuses.count)",
            "deleted": "\(deleted.count)",
            "deletedDeviceStatuses": "\(deletedDeviceStatuses.count)",
        ...
        return .init(
            ...
            tokenExpired: false,
            zoneMissing: false,
            upsertedDeviceStatuses: deviceStatuses,
            deletedDeviceStatusIDs: deletedDeviceStatuses)
    }
    ...
    private static func decodeDeviceStatusStatic(from record: CKRecord) -> SyncDeviceStatus? {
        guard let payload = record["payload"] as? Data else { return nil }
        if let version = record["encodingVersion"] as? Int,
           version > CloudSyncConstants.deviceStatusPayloadVersion
        {
            return nil
        }
        guard let json = try? PayloadCompression.decompress(payload) else { return nil }
        return try? CloudSyncConstants.makeJSONDecoder().decode(
            SyncDeviceStatus.self, from: json)
    }
    ...
    /// Fetches device-level status records from `DeviceProvidersZone`.
    /// Returns an empty list when the zone or record type does not exist yet.
    public func fetchDeviceStatuses() async -> [SyncDeviceStatus] {
        guard self.cloudKitAvailable, self._privateDatabase != nil else { return [] }
        ...
        do {
            ...
            self.logError("Device-status fetch failed: \(error.localizedDescription)")
            return []
        }

        var statuses: [SyncDeviceStatus] = []
        statuses.reserveCapacity(matchResults.count)
        for (recordID, result) in matchResults {
            switch result {
            case let .success(record):
                if let status = Self.decodeDeviceStatusStatic(from: record) {
                    statuses.append(status)
                } else {
                    self.logError(
                        "Failed to decode device status \(recordID.recordName)")
                }
            case let .failure(error):
                self.logError(
                    "Failed to fetch device status \(recordID.recordName): " +
                        error.localizedDescription)
            }
        }
        return statuses
