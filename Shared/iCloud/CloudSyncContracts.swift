...
public protocol SyncPushing: Sendable {
    ...
    /// Device-level status write (battery/power telemetry, no provider data).
    /// Stored separately so battery-only changes can reach iPhone even when
    /// provider payloads are unchanged and therefore diff-skipped.
    @discardableResult
...
extension SyncPushing {
    ...
    /// Default no-op so existing tests and non-CloudKit sync doubles do not
    /// need to care about optional device telemetry.
    public func pushDeviceStatus(_: SyncDeviceStatus) async -> SyncPushResult {
        .success
    }
