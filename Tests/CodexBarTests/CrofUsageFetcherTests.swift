...
struct CrofPluginGoldenTests {
    ...
    @Test
    func `half populated request quota is rejected`() async {
        await #expect(throws: (any Error).self) {
            try await Self.fetch(#"{"credits":10,"requests_plan":1000,"usable_requests":null}"#)
        }
        await #expect(throws: (any Error).self) {
            try await Self.fetch(#"{"credits":10,"requests_plan":null,"usable_requests":800}"#)
        }
    }
