...
struct CostUsageCustomPricingTests {
    ...
    @Test
    func `boolean and overflowing custom rates fail soft`() {
        let boolean = CostUsageCustomPricing.parse(Data("""
        { "gpt-5.4": { "input": false } }
        """.utf8))
        #expect(boolean.rates(model: "gpt-5.4") == nil)

        let numeric = CostUsageCustomPricing.parse(Data("""
        { "gpt-5.4": { "input": 0, "output": 1 } }
        """.utf8))
        #expect(numeric.rates(model: "gpt-5.4")?.input == 0)
        #expect(numeric.rates(model: "gpt-5.4")?.output == 1)

        let huge = CostUsageCustomPricing.parse(Data("""
        { "gpt-5.4": { "input": 1.7976931348623157e308 } }
        """.utf8))
        #expect(huge.costUSD(model: "gpt-5.4", inputTokens: Int.max, outputTokens: 0) == nil)
    }
