...
struct ClaudeBaselineCharacterizationTests {
    ...
    func `CLI auto reaches Claude executable after web credentials are unavailable`() async throws {
        ...
        let settings = ProviderSettingsSnapshot.make(claude: .init(
            ...
            manualCookieHeader: nil))
        let env = ["CLAUDE_CLI_PATH": stubCLIPath]
        let legacyCookieCache = FileManager.default.temporaryDirectory
            .appendingPathComponent("claude-cli-auto-cookie-cache-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: legacyCookieCache) }
        ...
        let outcome = await CookieHeaderCache.withLegacyBaseURLOverrideForTesting(legacyCookieCache) {
            await ClaudeWebFetchStrategy.$usageLoaderOverrideForTesting.withValue(webLoader) {
