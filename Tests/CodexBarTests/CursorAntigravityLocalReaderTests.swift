...
struct CursorAntigravityLocalReaderTests {
    ...
    @Test
    func `cursor date only rows stay in configured bucket calendar`() throws {
        let csv = [
            "Date,Cloud Agent ID,Automation ID,Kind,Model,Max Mode,"
                + "Input (w/ Cache Write),Input (w/o Cache Write),"
                + "Cache Read,Output Tokens,Total Tokens,Cost",
            "2026-08-20,agent-123,,agent,test-model-a,off,900,700,200,120,1020,0.05",
        ].joined(separator: "\n") + "\n"
        let url = try Self.writeTemporary(csv, extension: "csv")
        defer { try? FileManager.default.removeItem(at: url) }
        ...
        let row = try #require(CursorLocalCSVReader.parseFile(at: url, calendar: calendar).first)

        #expect(CostUsageLocalDay.key(from: row.date, calendar: calendar) == "2026-08-20")
    }
