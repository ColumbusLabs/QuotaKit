...
struct CodexSubagentOrdinalBoundaryTests {
        ...
        revision: Int?) async throws
    {
        ...
        let repairedRows = try #require(reopened.files[path]?.codexRows)
        #expect(repairedRows.isEmpty)
        ...
        clock: inout Date) throws -> CostUsageDailyReport
    {
        for _ in 0..<20 {
            ...
            let files = cache.files.values
            if !files.isEmpty, cache.codexScanCatchUpPending != true,
               files.allSatisfy({ $0.codexScanComplete == true && $0.hasCurrentCodexParser })
            {
                return report
            }
        }
