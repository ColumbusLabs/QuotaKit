...
struct LiteLLMUsageFetcherTests {
    @Test(arguments: LiteLLMPluginTestSupport.engines)
    func `bundled manifest parses without unsupported capabilities`(engine: ProviderPluginEngineKind) throws {
        let runtime = try LiteLLMPluginTestSupport.runtime(
            engine: engine,
            transport: ProviderHTTPTransportHandler { _ in throw URLError(.cancelled) })

        #expect(runtime.manifest.capabilities.isEmpty)
    }
    ...
    func `parses key info identity for user lookup`(engine: ProviderPluginEngineKind) async throws {
        ...
        #expect(snapshot.subscriptionExpiresAt == formatter.date(from: "2026-09-11T00:12:55.950000+00:00"))
    ...
    func `management urls accept root or v1 base urls`(engine: ProviderPluginEngineKind) async throws {
        for base in [
            ...
            _ = try await LiteLLMPluginTestSupport.runtime(engine: engine, transport: transport)
    ...
    func `fetch trims api key before sending management requests`(engine: ProviderPluginEngineKind) async throws {
        ...
        let snapshot = try await LiteLLMPluginTestSupport.runtime(engine: engine, transport: transport)
            .fetchUsage(
