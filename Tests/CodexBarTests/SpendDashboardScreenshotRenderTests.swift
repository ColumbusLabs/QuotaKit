...
final class SpendDashboardScreenshotRenderTests: XCTestCase {
    ...
    func test_renderCivilDayCoverageAcrossMidnightDST() throws {
        ...
        let snapshot = Self.snapshot(
            ...
            historyDays: 2,
            now: now)
        let selectedDay = try XCTUnwrap(calendar.date(byAdding: .day, value: -1, to: now))
        for (name, selected) in [("coverage", Date?.none), ("selected-day", Optional(selectedDay))] {
