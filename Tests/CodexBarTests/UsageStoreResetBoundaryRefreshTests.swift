...
struct UsageStoreResetBoundaryRefreshTests {
    ...
    @Test
    func `stale snapshot after boundary refresh remains retryable`() {
        let now = Date(timeIntervalSince1970: 2775)
        let resetsAt = now.addingTimeInterval(-3 * 60)
        let boundaryRefreshAt = resetsAt.addingTimeInterval(UsageStore.resetBoundaryRefreshGraceSeconds)
        let snapshot = Self.snapshot(
            updatedAt: resetsAt.addingTimeInterval(-60),
        ...
        #expect(UsageStore.shouldRecordResetBoundaryAttempt(
            isRefreshing: false,
            snapshots: [.codex: snapshot],
            boundaryRefreshAt: boundaryRefreshAt) == false)
    ...
    @Test
    func `refreshed snapshot after boundary suppresses repeat boundary refresh`() {
        let now = Date(timeIntervalSince1970: 2785)
        let resetsAt = now.addingTimeInterval(-3 * 60)
        let boundaryRefreshAt = resetsAt.addingTimeInterval(UsageStore.resetBoundaryRefreshGraceSeconds)
        let snapshot = Self.snapshot(
            updatedAt: boundaryRefreshAt.addingTimeInterval(1),
            primaryResetsAt: resetsAt)

        #expect(UsageStore.shouldRecordResetBoundaryAttempt(
            isRefreshing: false,
            snapshots: [.codex: snapshot],
            boundaryRefreshAt: boundaryRefreshAt) == true)
    }
