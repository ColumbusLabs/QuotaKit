...
struct SpendDashboardPartialCostTests {
    ...
    @Test
    func `Cursor lower-bound spend is visibly partial when a request has no cost`() throws {
        let entry = CostUsageDailyReport.Entry(
            ...
            requestCount: 2,
            costUSD: 1,
            modelsUsed: ["claude-4.5-sonnet", "gpt-5"],
            modelBreakdowns: [
                .init(modelName: "claude-4.5-sonnet", costUSD: 1, totalTokens: 5, requestCount: 1),
                .init(modelName: "gpt-5", costUSD: nil, totalTokens: 7, requestCount: 1),
            ],
        ...
        #expect(group.coverage == CostUsageCoverageCounts(priced: 1, unpriced: 1))
