...
struct ZaiUsageSnapshotTests {
    ...
    func `maps usage snapshot windows`() {
        ...
        #expect(usage.extraRateWindows?.first?.window.windowMinutes == 30 * 24 * 60)
    ...
    @Test
    func `time limit preserves its explicit MCP duration`() {
        let reset = Date(timeIntervalSince1970: 123)
        let timeLimit = ZaiLimitEntry(
            type: .timeLimit,
            unit: .hours,
...
struct ZaiUsageParsingTests {
    ...
    func `parses usage response`() throws {
        ...
        #expect(
            usage.extraRateWindows?.first?.window.windowMinutes
