...
struct CodexLocalProjectUsageTests {
    ...
    @Test
    func `workspace sidecar defaults to the shared application cache root`() throws {
        let cachesDirectory = try #require(FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).first)
        let expected = cachesDirectory
        ...
        #expect(CodexWorkspaceUsageSidecar.databaseURL(cacheRoot: nil) == expected)
    }
