...
struct UsagePaceTextTests {
    ...
    func `on track pace detail still reports rounded visible delta`() {
        ...
        #expect(UsagePaceText.weeklyDetail(provider: .codex, pace: deficit, now: now).leftLabel == "1% in deficit")
        #expect(UsagePaceText.weeklyDetail(provider: .codex, pace: reserve, now: now).leftLabel == "1% in reserve")
        #expect(UsagePaceText.weeklyDetail(provider: .codex, pace: roundedZero, now: now).leftLabel == "On pace")
    }
