...
struct SpendDashboardExportTests {
    ...
    func `export filename uses the selected window`() {
        #expect(SpendDashboardJSONExporter.defaultFilename(days: 7) == "quotakit-spend-last-7-days.json")
        #expect(SpendDashboardJSONExporter.defaultFilename(days: 30) == "quotakit-spend-last-30-days.json")
        #expect(
            SpendDashboardJSONExporter.defaultFilename(days: SpendDashboardSource.scanDays)
                == "quotakit-spend-all-time.json")
    }
    ...
    func `save writes a file and cancel does not`() throws {
        ...
        let savedURL = root.appendingPathComponent("quotakit-spend-last-7-days.json")
        let saved = SpendDashboardJSONExporter.save(
            model: model,
            hiddenSourceIDs: ["cursor"],
            chooseDestination: { filename in
                #expect(filename == "quotakit-spend-last-7-days.json")
                return savedURL
            })
        #expect(saved)
