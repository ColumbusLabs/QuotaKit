...
struct SpendDashboardMidnightDSTTests {
        ...
        completeHistory: Bool) throws
    {
        ...
        let now = try #require(calendar.date(from: DateComponents(
            ...
            day: septemberDay,
            hour: 12)))
        let dates = try (0..<SpendActivitySeries.rangeDayCount).map { offset in
        ...
        let totalHistoryTokens = (1...SpendActivitySeries.rangeDayCount).reduce(0, +)
        let snapshot = CostUsageTokenSnapshot(
            sessionTokens: nil,
            sessionCostUSD: nil,
            last30DaysTokens: completeHistory ? totalHistoryTokens : nil,
            last30DaysCostUSD: nil,
            historyDays: SpendActivitySeries.rangeDayCount,
        ...
        for requestedDays in [7, 30, 365] {
            ...
            #expect(group.dailyPoints.map(\.day) == Array(dates.prefix(requestedDays).reversed()))
            #expect(group.totalTokens == (1...requestedDays).reduce(0, +))
            if completeHistory {
                #expect(group.coveredDayCount == requestedDays)
                #expect(group.providers.first?.coveredDayCount == requestedDays)
                #expect(group.dailyPoints.count == requestedDays)
            }
            ...
            #expect(series.daily.reduce(0, +) == totalHistoryTokens)
        }
    }
