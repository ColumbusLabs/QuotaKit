    ...
    private struct Fixture {
        ...
        init() throws {
            self.root = FileManager.default.temporaryDirectory
                .appendingPathComponent("ClaudeSwapSwitchErrorTimingTests-\(UUID())", isDirectory: true)
            try FileManager.default.createDirectory(at: self.root, withIntermediateDirectories: true)
            self.settings = testSettingsStore(suiteName: "ClaudeSwapSwitchErrorTimingTests")
            self.settings.refreshFrequency = .manual
            self.settings.statusChecksEnabled = false
            self.store = UsageStore(
                fetcher: UsageFetcher(environment: ["HOME": self.root.path]),
                browserDetection: BrowserDetection(cacheTTL: 0),
                settings: self.settings,
                startupBehavior: .testing,
                environmentBase: ["HOME": self.root.path])
        }
