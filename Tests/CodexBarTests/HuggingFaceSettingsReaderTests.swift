...
struct HuggingFaceSettingsReaderTests {
    ...
    func `config key takes precedence over vendor environment keys`() {
        let environment = [
            "QUOTAKIT_HUGGINGFACE_API_KEY": "hf_config",
            "HF_TOKEN": "hf_env",
    ...
    @Test
    func `rejects token paths that are not regular files`() throws {
        let directory = try Self.makeTempDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }

        #expect(HuggingFaceSettingsReader.apiKey(
            environment: ["HF_TOKEN_PATH": directory.path],
            homeDirectory: Self.emptyHome()) == nil)
    }

    @Test
    func `rejects oversized token files before reading their contents`() throws {
        let directory = try Self.makeTempDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let tokenFile = directory.appendingPathComponent("oversized-token")
        let oversizedFixture = String(repeating: "x", count: HuggingFaceSettingsReader.maximumTokenFileBytes + 1)
        try oversizedFixture.write(to: tokenFile, atomically: true, encoding: .utf8)

        #expect(HuggingFaceSettingsReader.apiKey(
            environment: ["HF_TOKEN_PATH": tokenFile.path],
            homeDirectory: Self.emptyHome()) == nil)
    }
