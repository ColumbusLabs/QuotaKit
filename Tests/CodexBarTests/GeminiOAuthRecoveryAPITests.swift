...
struct GeminiOAuthRecoveryAPITests {
    ...
    @Test
    func `refreshes via known Homebrew Cellar libexec path when gemini binary lacks oauth config`() async throws {
        let env = try GeminiTestEnvironment()
        defer { env.cleanup() }
        try env.writeCredentials(
            accessToken: "old-token",
    ...
    @Test
    func `refreshes via known Homebrew Cellar libexec path when gemini binary is unavailable`() async throws {
        let env = try GeminiTestEnvironment()
        defer { env.cleanup() }
        try env.writeCredentials(
            accessToken: "old-token",
        ...
        try Self.plantHomebrewCellarGeminiPackage(
            under: homebrewPrefix,
            clientID: "cellar-no-binary-id",
            clientSecret: "cellar-no-binary-secret")
        ...
        let dataLoader = GeminiAPITestHelpers.dataLoader { request in
            ...
            case "oauth2.googleapis.com":
                let body = request.httpBody.flatMap { String(data: $0, encoding: .utf8) } ?? ""
                guard body.contains("client_id=cellar-no-binary-id") else {
                    return GeminiAPITestHelpers.response(url: url.absoluteString, status: 400, body: Data())
                }
                let json = GeminiAPITestHelpers.jsonData([
                    "access_token": "new-token",
        ...
        let snapshot = try await GeminiOAuthConfig.$environmentOverride.withValue(clearOAuthEnv) {
            try await GeminiStatusProbe.$skipInstalledGeminiDiscoveryForTesting.withValue(true) {
                try await GeminiStatusProbe.$knownInstallPrefixesForTesting.withValue([homebrewPrefix.path]) {
                    try await probe.fetch()
                }
            }
