...
struct CLIServeAuthTests {
    ...
    func `serve help documents dashboard token transport honestly`() {
        ...
        #expect(serve.contains("QUOTAKIT_DASHBOARD_TOKEN"))
    ...
    func `dashboard token resolution prefers the environment and rejects blanks`() {
        ...
        #expect(CodexBarCLI.resolveDashboardToken(
            from: emptyValues,
            environment: ["QUOTAKIT_DASHBOARD_TOKEN": "  "])
            == .empty(source: "QUOTAKIT_DASHBOARD_TOKEN"))
        #expect(CodexBarCLI.resolveDashboardToken(
            from: emptyValues,
            environment: ["CODEXBAR_DASHBOARD_TOKEN": "legacy"])
            == .token("legacy"))
        #expect(CodexBarCLI.resolveDashboardToken(
