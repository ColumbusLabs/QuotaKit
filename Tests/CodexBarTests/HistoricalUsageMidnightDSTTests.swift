...
struct HistoricalUsageMidnightDSTTests {
    ...
    func `backfill retains credits and civil day boundaries`(dateCase: (String, Int, Int)) throws {
        ...
        let breakdown = [(formatter.string(from: first), 23.0), (formatter.string(from: asOf), 12.0)].map {
            OpenAIDashboardDailyBreakdown(day: $0.0, services: [], totalCreditsUsed: $0.1)
        }
        func credits(_ start: Date, _ end: Date) -> Double {
            HistoricalUsageHistoryStore._creditsUsedForTesting(
                breakdown: breakdown, asOf: asOf, start: start, end: end, calendar: calendar)
        }
        #expect(abs(credits(firstStart, asOf) - 35) < 0.001)
        #expect(abs(credits(firstStart, secondStart) - 23) < 0.001)
        #expect(abs(credits(secondStart, asOf) - 12) < 0.001)
    }
}
