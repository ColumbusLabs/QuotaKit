...
struct ProviderPluginParityTests {
    ...
    func `QuotaKit compatibility providers keep native defaults and prepend JS when flagged`() async {
        for (provider, key, nativeID) in [
            ...
            let defaultContext = Self.context(environment: environment)
            let defaultStrategies = await descriptor.fetchPlan.pipeline.resolveStrategies(defaultContext)
            #expect(defaultStrategies.map(\.id) == [nativeID])

            environment[ProviderPluginPrototype.environmentKey] = "1"
            let flaggedContext = Self.context(environment: environment)
            let flagged = await descriptor.fetchPlan.pipeline.resolveStrategies(flaggedContext)
            #expect(flagged.map(\.id) == ["\(provider.rawValue).js", nativeID])
            #expect(await flagged[0].isAvailable(flaggedContext))
        }
    }
