...
struct OpenCodexUsageParserTests {
    ...
    @Test
    func `rejects unsafe and fractional token counts without trapping`() throws {
        let line = """
        {"requestId":"limits","timestamp":1784179200,"provider":"openai","model":"gpt-5.4",\
        "usageStatus":"reported","usage":{"inputTokens":9223372036854775808,"outputTokens":1.5},\
        "totalTokens":9223372036854775808}
        """
        let entry = try #require(OpenCodexUsageParser.parseLine(line))
        #expect(entry.usage == nil)
        #expect(entry.totalTokens == nil)
        #expect(entry.resolvedTotalTokens == nil)

        let small = try #require(OpenCodexUsageParser.parseLine("""
        {"requestId":"small","timestamp":1784179200,"provider":"openai","model":"gpt-5.4",\
        "usage":{"inputTokens":0,"outputTokens":1}}
        """))
        #expect(small.usage?.inputTokens == 0)
        #expect(small.usage?.outputTokens == 1)
    }

    @Test
    func `overflowing token components stay unresolved and aggregate safely`() throws {
        ...
        let entries = [
            OpenCodexUsageEntry(
                ...
                usage: OpenCodexTokenUsage(inputTokens: Int.max, totalTokens: Int.max),
                totalTokens: Int.max),
            OpenCodexUsageEntry(
                requestID: "one",
                timestamp: now,
                ...
                model: "gpt-5.4",
                usageStatus: .reported,
                usage: OpenCodexTokenUsage(inputTokens: 1, totalTokens: 1),
                totalTokens: 1),
        ]
