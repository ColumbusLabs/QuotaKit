...
struct CostUsageScannerTests {
    ...
    func `codex incremental parsing keeps current turn id`() throws {
        // Upstream v0.26.x regression: when an incremental Codex scan
        // continues from a prior `parsedBytes` offset, the parser must
        // retain the last seen `task_started.id` so subsequent
        // token_count rows attribute their delta to the right turn.
        let env = try CostUsageTestEnvironment()
        defer { env.cleanup() }
