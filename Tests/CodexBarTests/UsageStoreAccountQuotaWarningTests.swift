...
struct UsageStoreAccountQuotaWarningTests {
    ...
    func `ordinary refresh keeps selected token account warning episodes independent`() async {
        ...
        // Exercise the selected-account path. QuotaKit's iCloud fan-out intentionally fetches
        // every token account, which is covered separately by account-snapshot tests.
