...
extension StatusMenuTests {
    ...
    @Test
    func `credits hint copy affordance stays enabled without highlighting the card`() {
        StatusItemController.setMenuRefreshEnabledForTesting(false)
        let previousRendering = StatusItemController.menuCardRenderingEnabled
        StatusItemController.menuCardRenderingEnabled = true
        defer { StatusItemController.menuCardRenderingEnabled = previousRendering }
        ...
        let model = UsageMenuCardView.Model(
            ...
            creditsHintText: "OpenAI dashboard signed in as user@example.com.",
            creditsHintCopyText: "OpenAI dashboard signed in as user@example.com.",
            providerCost: nil,
            tokenUsage: nil,
            placeholder: nil,
            progressColor: .blue)
        ...
        #expect(item.isEnabled)
        #expect(controller.highlightedMenuItems[ObjectIdentifier(menu)] == nil)
        guard let hosting = item
            .view as? MenuCardItemHostingView<MenuCardSectionContainerView<UsageMenuCardCreditsSectionView>>
        else {
            Issue.record("expected a credits card hosting view")
            return
        }
        #expect(!hosting.allowsMenuHighlight)
        #expect(!hosting.highlightState.isHighlighted)
